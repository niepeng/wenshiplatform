...
function city(provice, key, area) {
	  var index;
	 var cnt = provinces.length;
	 for (i = 0; i < cnt; i++) {
        if (provice == provinces[i]) {
            index = i;
            break;
        }
    }
	...
	if (dsy.Exists(str)) {
		...
		// s3.options.add(new Option(opt0[2], opt0[2]));
		for (i = 0; i < ar.length; i++){
			s3.options.add(new Option(ar[i], ar[i]));
		}
		
		for(var i=0; i<s3.options.length; i++){  
			if(s3.options[i].innerHTML == area){  
